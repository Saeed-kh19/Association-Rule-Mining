import pandas as pd
from loading import retail_data  # Import the dataset from loading.py

# Remove rows with non-positive Quantity
filtered_by_quantity = retail_data[retail_data['Quantity'] > 0]

# Remove rows with non-positive UnitPrice
filtered_by_price = filtered_by_quantity[filtered_by_quantity['UnitPrice'] > 0]

rows_removed_due_to_quantity = len(retail_data) - len(filtered_by_quantity)
rows_removed_due_to_price = len(filtered_by_quantity) - len(filtered_by_price)
total_rows_removed = len(retail_data) - len(filtered_by_price)
